...
func (s *Statistics) AddTuple(vals []string) {
	s.Size++
	for i, v := range vals {
		freq := s.Hgrams[i].Frequency(v)
		if s.Hgrams[i].Update(v, freq+1) {
			s.Ndv[i]++
		}
	}
}
...
func EstimateJoinSize(tables []*Statistics) (int, *Statistics) {
	...
	// todo mixed case
	panic("not ready for mixed statistics")
	return -1, &Statistics{}
}
...
func EstimateSelectionSize(tab *Statistics, cond Condition) (int, *Statistics) {
	return 0, nil
}
